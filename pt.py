...
brightnessSlider = tk.Scale(Frame2, bg="#8ED2C9", fg="black",
                            troughcolor="#006495", label="Brightness",
                            from_=0, to=2, orient=tk.HORIZONTAL,
                            length=screen_width, resolution=0.1,
                            command=brightness_callback)
brightnessSlider.set(1)
brightnessSlider.pack(anchor=tk.N)

contrastSlider = tk.Scale(Frame2, bg="#8ED2C9", fg="black",
                          troughcolor="#006495", label="Contrast", from_=0,
                          to=2, orient=tk.HORIZONTAL, length=screen_width,
                          command=contrast_callback, resolution=0.1)
contrastSlider.set(1)
contrastSlider.pack(anchor=tk.N)

ColorSlider = tk.Scale(Frame2, bg="#8ED2C9", fg="black", troughcolor="#006495",
                       label="Saturation", from_=0, to=2, orient=tk.HORIZONTAL,
                       length=screen_width, resolution=0.1,
...
pinkButton = tk.Radiobutton(Frame3, bg="#EE1289", text="Pink Filter",
                            font=("ariel 10 bold"), width=30, value=3,
                            command=pinkButton_callback)
pinkButton.grid(row=0, column=2)

orangeButton = tk.Radiobutton(Frame3, bg="#FF7D40", text="Orange Filter",
                              font=("ariel 10 bold"), width=30, value=4,
                              command=orangeButton_callback)
orangeButton.grid(row=0, column=3)

noneButton = tk.Radiobutton(Frame3, bg="#474747", fg="white", text="None",
                            width=30, font=("ariel 10 bold"), value=5,
                            command=noneButton_callback)
